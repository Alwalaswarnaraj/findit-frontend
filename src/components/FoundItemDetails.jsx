...
const FoundItemDetails = () => {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        const res = await api.get(`http://localhost:5000/api/found/${id}`);
        setItem(res.data);
        setLoading(false);
      } catch (err) {
        ...
        <div className="flex-1">
          ...
          {token && item.user?._id !== currentUserId && (
            <button
              ...
              className="mt-6 bg-blue-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 cursor-pointer transition"
            >
